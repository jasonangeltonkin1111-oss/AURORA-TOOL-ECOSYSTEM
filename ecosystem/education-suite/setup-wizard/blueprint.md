...
- **Public tier vs private tier:** Public = Static wizard; Private = Adaptive
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Guide setup with predictable behavior that reduces manual errors.
- **Target user:** New users configuring AURORA tools.
- **Primary use cases:** Use Setup Wizard to onboard users and codify repeatable operating templates.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Static wizard** behavior, while private roadmap variant extends to **Adaptive**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for guide setup, footer with recent actions/log snippets.
