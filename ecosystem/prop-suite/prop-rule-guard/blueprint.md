...
## 1. Tool identity
- **Name:** Prop Rule Guard
- **Suite:** Prop Suite
- **Public tier vs private tier:** Public = Fixed rules; Private = Dynamic engine
- **Status (Idea/Planned/In Build/Released):** In Build

## 2. User-facing definition
- **Problem statement:** Stay within rules with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Prop Rule Guard to avoid challenge/account rule breaches and track progress requirements.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Fixed rules** behavior, while private roadmap variant extends to **Dynamic engine**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for stay within rules, footer with recent actions/log snippets.
