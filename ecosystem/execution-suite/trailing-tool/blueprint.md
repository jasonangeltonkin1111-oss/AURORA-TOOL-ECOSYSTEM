...
- **Public tier vs private tier:** Public = Fixed trail; Private = Volatility-aware
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Trail SL with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Trailing Tool to adjust SL/TP logic quickly after entry while preserving predefined risk rules.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Fixed trail** behavior, while private roadmap variant extends to **Volatility-aware**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for trail sl, footer with recent actions/log snippets.
