# Daily Loss Guard Blueprint

## 1. Tool identity
- **Name:** Daily Loss Guard
- **Suite:** Risk Suite
- **Public tier vs private tier:** Public = Fixed threshold; Private = Dynamic DD logic
- **Status (Idea/Planned/In Build/Released):** In Build

## 2. User-facing definition
- **Problem statement:** Stop trading after loss with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Daily Loss Guard to size positions, enforce account risk caps, and standardize execution risk before order send.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Fixed threshold** behavior, while private roadmap variant extends to **Dynamic DD logic**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for stop trading after loss, footer with recent actions/log snippets.
...
## 8. Release metadata
- **Version:** 0.1.0-daily-loss-guard
