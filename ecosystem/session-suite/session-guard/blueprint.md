# Session Guard Blueprint

## 1. Tool identity
- **Name:** Session Guard
- **Suite:** Session Suite
- **Public tier vs private tier:** Public = Simple filter; Private = Adaptive windows
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Filter trading with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Session Guard to align trading activity to valid sessions and avoid off-hours degradation.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Simple filter** behavior, while private roadmap variant extends to **Adaptive windows**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for filter trading, footer with recent actions/log snippets.
...
- **Session/news/time restrictions:** Session/time filters are evaluated in account timezone and broker server timezone.
