# Exposure Meter Blueprint

## 1. Tool identity
- **Name:** Exposure Meter
- **Suite:** Risk Suite
- **Public tier vs private tier:** Public = Basic sum; Private = Portfolio-aware
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Show total risk with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Exposure Meter to size positions, enforce account risk caps, and standardize execution risk before order send.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Basic sum** behavior, while private roadmap variant extends to **Portfolio-aware**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for show total risk, footer with recent actions/log snippets.
