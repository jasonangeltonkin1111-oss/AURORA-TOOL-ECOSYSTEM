...
## 1. Tool identity
- **Name:** Discipline Lock
- **Suite:** Discipline Suite
- **Public tier vs private tier:** Public = Manual lock; Private = Behavior engine
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Lock trading with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Discipline Lock to reduce impulsive behavior with explicit pre/post-trade controls.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Manual lock** behavior, while private roadmap variant extends to **Behavior engine**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for lock trading, footer with recent actions/log snippets.
