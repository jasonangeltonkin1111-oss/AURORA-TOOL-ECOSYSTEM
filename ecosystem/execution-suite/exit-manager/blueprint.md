# Exit Manager Blueprint

## 1. Tool identity
- **Name:** Exit Manager
- **Suite:** Execution Suite
- **Public tier vs private tier:** Public = Fixed targets; Private = Adaptive
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Multi TP exits with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Exit Manager to adjust SL/TP logic quickly after entry while preserving predefined risk rules.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Fixed targets** behavior, while private roadmap variant extends to **Adaptive**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for multi tp exits, footer with recent actions/log snippets.
