# Dashboard Lite Blueprint

## 1. Tool identity
- **Name:** Dashboard Lite
- **Suite:** Dashboard Suite
- **Public tier vs private tier:** Public = Basic stats; Private = Full telemetry
- **Status (Idea/Planned/In Build/Released):** In Build

## 2. User-facing definition
- **Problem statement:** Overview panel with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Dashboard Lite to maintain account + market awareness without switching windows.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Basic stats** behavior, while private roadmap variant extends to **Full telemetry**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for overview panel, footer with recent actions/log snippets.
