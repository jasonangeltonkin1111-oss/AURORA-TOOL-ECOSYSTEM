...
- **Name:** Session Clock
- **Suite:** Session Suite
- **Public tier vs private tier:** Public = Basic display; Private = True session logic
- **Status (Idea/Planned/In Build/Released):** In Build

## 2. User-facing definition
- **Problem statement:** Show sessions with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Session Clock to align trading activity to valid sessions and avoid off-hours degradation.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Basic display** behavior, while private roadmap variant extends to **True session logic**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for show sessions, footer with recent actions/log snippets.
