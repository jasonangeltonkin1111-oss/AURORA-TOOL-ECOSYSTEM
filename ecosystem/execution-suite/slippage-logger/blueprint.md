...
## 1. Tool identity
- **Name:** Slippage Logger
- **Suite:** Execution Suite
- **Public tier vs private tier:** Public = Log only; Private = Analytics engine
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Track slippage with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Slippage Logger to execute or gate orders faster with lower operational mistakes.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Log only** behavior, while private roadmap variant extends to **Analytics engine**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for track slippage, footer with recent actions/log snippets.
