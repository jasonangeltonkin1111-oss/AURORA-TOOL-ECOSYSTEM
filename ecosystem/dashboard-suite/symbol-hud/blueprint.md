# Symbol HUD Blueprint

## 1. Tool identity
- **Name:** Symbol HUD
- **Suite:** Dashboard Suite
- **Public tier vs private tier:** Public = Simple stats; Private = Full dossier
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Symbol info with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Symbol HUD to maintain account + market awareness without switching windows.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Simple stats** behavior, while private roadmap variant extends to **Full dossier**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for symbol info, footer with recent actions/log snippets.
