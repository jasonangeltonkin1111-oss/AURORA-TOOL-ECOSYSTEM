# News Guard Blueprint

## 1. Tool identity
- **Name:** News Guard
- **Suite:** Dashboard Suite
- **Public tier vs private tier:** Public = Manual windows; Private = Event intelligence
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Avoid news with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use News Guard to monitor critical conditions and notify before violations or missed states.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Manual windows** behavior, while private roadmap variant extends to **Event intelligence**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for avoid news, footer with recent actions/log snippets.
...
- **Session/news/time restrictions:** News blackout windows block/confirm actions around high-impact events.
