# Challenge Dashboard Blueprint

## 1. Tool identity
- **Name:** Challenge Dashboard
- **Suite:** Prop Suite
- **Public tier vs private tier:** Public = Basic tracking; Private = Scenario engine
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Track challenge with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Challenge Dashboard to avoid challenge/account rule breaches and track progress requirements.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Basic tracking** behavior, while private roadmap variant extends to **Scenario engine**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for track challenge, footer with recent actions/log snippets.
...
- **Session/news/time restrictions:** Prop hard limits are enforced pre-order with strictest-rule-wins behavior.
...
## 8. Release metadata
- **Version:** 0.1.0-challenge-dashboard
