# Cooldown Timer Blueprint

## 1. Tool identity
- **Name:** Cooldown Timer
- **Suite:** Discipline Suite
- **Public tier vs private tier:** Public = Fixed timer; Private = Adaptive
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Prevent revenge trades with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Cooldown Timer to reduce impulsive behavior with explicit pre/post-trade controls.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Fixed timer** behavior, while private roadmap variant extends to **Adaptive**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for prevent revenge trades, footer with recent actions/log snippets.
