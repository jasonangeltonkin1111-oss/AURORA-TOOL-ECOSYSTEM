...
- **Public tier vs private tier:** Public = Fixed trigger; Private = ATR/adaptive
- **Status (Idea/Planned/In Build/Released):** In Build

## 2. User-facing definition
- **Problem statement:** Move SL with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Breakeven Tool to adjust SL/TP logic quickly after entry while preserving predefined risk rules.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Fixed trigger** behavior, while private roadmap variant extends to **ATR/adaptive**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for move sl, footer with recent actions/log snippets.
