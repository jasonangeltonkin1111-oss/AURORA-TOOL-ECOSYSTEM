# Profit Lock Blueprint

## 1. Tool identity
- **Name:** Profit Lock
- **Suite:** Risk Suite
- **Public tier vs private tier:** Public = Fixed trigger; Private = Adaptive trailing
- **Status (Idea/Planned/In Build/Released):** Planned

## 2. User-facing definition
- **Problem statement:** Lock profits with predictable behavior that reduces manual errors.
- **Target user:** Retail and prop traders.
- **Primary use cases:** Use Profit Lock to size positions, enforce account risk caps, and standardize execution risk before order send.
...
- **Core logic rules:** Deterministic rule evaluation; public logic follows **Fixed trigger** behavior, while private roadmap variant extends to **Adaptive trailing**.
...
## 4. UX specification
- **Panel layout:** Header (status + symbol/account), central control group for lock profits, footer with recent actions/log snippets.
